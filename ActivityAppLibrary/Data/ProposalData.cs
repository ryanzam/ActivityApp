...
namespace ActivityAppLibrary.Data
{
    public class ProposalData : IProposalData
    {
        ...
        public async Task<List<ProposalModel>> GetApprovedProposals()
        {
            ...
            foreach (ProposalModel p in data)
            {
                if (p.ActivityDate < DateTime.Now)
                {
        ...
        public async Task<List<ProposalModel>> GetUsersProposals(string uid)
        {
            var res = _memoryCache.Get<List<ProposalModel>>(uid);
            if (res is null)
            {
                var response = await proposals.FindAsync(p => p.Proposer.Id == uid);
                res = response.ToList();
                _memoryCache.Set(uid, res, TimeSpan.FromMinutes(1));
            }
            return res;
        }
        ...
        public async Task UpdateProposal(ProposalModel proposal)
        {
        ...
        public async Task ThumbsUpProposal(string propId, string usrId)
        {
            ...
            try
            {
                ...
                if (isThumbsUp) { usr.ProposalsThumbsUpped.Add(new SimpleProposalModel(prop)); }
                else
                {
                    var removeProp = usr.ProposalsThumbsUpped.Where(p => p.Id == propId).First();
                    usr.ProposalsThumbsUpped.Remove(removeProp);
                }
                await usrsInTransac.ReplaceOneAsync(session, u => u.Id == usrId, usr);
